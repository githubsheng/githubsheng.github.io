 ...
 */
class CircleType {
    constructor(image, mainColor, radius, explosionCreator) {
        this.image = image;
        this.mainColor = mainColor;
        this.radius = radius;
        this.explosionCreator = explosionCreator || getColorfulExplosion
    }
}
...
function initCircleTypes(imgAssets) {
    return [
        ...
        new CircleType(imgAssets.get('x2'), '#FFBD02', 10, getYellowStarExplosion),
        new CircleType(imgAssets.get('bonus'), '#D9061B', 10, getPinkStarExplosion),
        new CircleType(imgAssets.get('bonus2'), '#D9061B', 10, getPinkStarExplosion),
    ];
}